...
def bottom_up_merging_cam(all_mask_to_lines3d, threshold=0.5):
    def merging(all_mask_to_lines3d, threshold=0.5):    
        merged_results = all_mask_to_lines3d.copy()
        while len(merged_results) > 1:
            # 获取排序后的键
            cam_ids = sorted(merged_results.keys())
            ...
            print(f"After merging, number of views: {len(merged_results)}")
        return merged_results[0]
    cam_num = len(all_mask_to_lines3d) / 5
    partial_merged_results = {}
    for i in range(5):
        # 取出部分视角进行合并
        partial_all_mask_to_lines3d = {}
        for j in range(int(cam_num)):
            cam_id = i * int(cam_num) + j
            if cam_id in all_mask_to_lines3d:
                partial_all_mask_to_lines3d[cam_id] = all_mask_to_lines3d[cam_id]
        partial_merged_result = merging(partial_all_mask_to_lines3d, threshold)
        partial_merged_results[i] = partial_merged_result
    merged_results = merging(partial_merged_results)
    # 剔除小于3个线段的聚类
    merged_results = {k: list(v) for k, v in merged_results.items() if len(v) >= 3}
